  ...
  with (processingInstance) {
        ...
        draw: function () {
          ...
          switch (this.content) {
            ...
            case "replay":
              ...
              noStroke();
              fill(app.colors.stroke);
              translate(this.diameter * 0.3, -this.diameter * 0.18);
              rotate(radians(-70));
              triangle(
                0,
                -this.diameter * 0.1,
    ...
    var App = (function () {
      var App = function () {
        ...
        this.sounds = {
          lava: this.addSound(
            "https://www.kasandbox.org/proramming-sounds/rpg/water-bubble.mp3"
          ),
          bird: this.addSound(
            "https://www.kasandbox.org/proramming-sounds/rpg/giant-yah.mp3"
          ),
          spike: this.addSound(
            "https://www.kasandbox.org/proramming-sounds/rpg/giant-hyah.mp3"
          ),
          wall: this.addSound(
            "https://www.kasandbox.org/proramming-sounds/rpg/hit-thud.mp3"
          ),
          lazer: this.addSound(
            "https://www.kasandbox.org/proramming-sounds/retro/laser2.mp3"
          ),
          star: this.addSound(
            "https://www.kasandbox.org/proramming-sounds/rpg/metal-chime.mp3"
          ),
      ...
      App.prototype = {
        ...
        setBlocks: function () {
          ...
          for (var i = 0; i < 8; i++) {
            ...
            var blockColor =
              this.colors.blockColor[random(this.colors.blockColor.length) | 0];
            ...
            if (random() < 0.3) {
              ...
              var asset1 =
                this.assetImages[random(this.assetImages.length) | 0];
        ...
        star: function (x, y, inner, outer, sides) {
          ...
          for (var angle = 0; angle < 360; angle += rot / 2) {
            vertex(
              cos(radians(angle)) * (angle % rot === 0 ? outer : inner),
              sin(radians(angle)) * (angle % rot === 0 ? outer : inner)
            );
          }
          endShape(CLOSE);
          popMatrix();
          ...
          this.images = {
            ...
            plant3: function () {
              ...
              for (var i = 0; i < 360; i += 180) {
                ...
                line(diameter * 0.3, -diameter * 0.4, diameter * 0.3, 0);
                line(diameter * 0.4, diameter * 0.4, diameter * 0.4, 0);
              }
              popMatrix();
              popStyle();
        ...
        collisionBlocks: function(dir, blocks){
          for(var i = 0; i < blocks.length; i++){
            ...
            if(this.rectRectCollision(this.player, block)){
              ...
              this.player.eyes.value = random(this.player.eyes.max) | 0;
              this.player.mouth.value = random(this.player.mouth.max) | 0;
        ...
        movePlayer: function() {
          if (pressed) {
            ...
            this.player.vx = constrain(this.player.vx + 0.5 * this.player.dir, -3, 3);


            if (this.player.vy > 0) {
              this.player.vy = 0;
            }
            this.player.vy = constrain(this.player.vy -3, -8, this.player.h * 0.25);
          }
          if (this.start) {
            ...
            this.player.vy = constrain(this.player.vy + this.player.gravity, -8, this.player.h * 0.25);
        ...
        addSmoke: function() {
          for(var i = 0; i < 30; i++) {
            this.smokes.push({
              x: random(this.player.x, this.player.x + this.player.w),
              y: random(this.player.y, this.player.y + this.player.h),
              vx: random(-0.5, 0.5),
              vy: random(-1, -0.5),
              color: random(50, 80),
              diameter: 15,
        ...
        runSmoke: function() {
          ...
          for(var i = this.smokes.length - 1; i>= 0; i--){
            ...
            rect(smoke.x, smoke.y, smoke.diameter, smoke.diameter, 2);
        ...
        runBubbles: function() {
          ...
          for(var i = this.bubbles.length - 1; i >= 0; i--){
            ...
            bubble.x+= sin(radians(frameCount + bubble.offset))* bubble.vx;
            bubble.y+= bubble.vy;

            if(bubble.dir === 1) {
              bubble.opacity = constrain(bubble.opacity - 0.5, 0, 150);
              if(bubble.opacity === 0){
                bubble.dir*= -1;
              }
            }
            ...
            if(bubble.y - bubble.diameter > this.player.ymin + 400) {
              bubble.y-=800;
            }
            else if(bubble.y + bubble.diameter < this.player.ymin - 400){
              bubble.y+=800;
            }
          }
        },
        runFishes: function(){
          ...
          for(var i = 0; i < this.fishes.length; i++){
            ...
            pushMatrix();
            translate(fish.x, fish.y + sin(radians(frameCount * fish.speed)) * 20);
            rotate(radians((fish.dir === 1 ? 0 : 180)));
            ellipse(0, 0, fish.diameter, fish.diameter * 0.7);
            triangle(-fish.diameter * 0.4, 0, -fish.diameter * 0.7,
              -fish.diameter * 0.3, -fish.diameter * 0.7, fish.diameter * 0.3
            );
            popMatrix();

            fish.x+= fish.vx * fish.dir;

            if((fish.dir === 1 && fish.x - fish.diameter > width) || (fish.dir === -1 && fish.x + fish.diameter < 0)) {
              var dir = random() < 0.5 ? 1 : -1;

              if(dir === 1){fish.x = -fish.diameter;}
              else {fish.x = width + fish.diameter;}
              fish.y = random(height - 50) - this.player.ymin;
            }
            else if(fish.y - fish.diameter > this.player.ymin + 400){
              fish.y-= 800;
            }
            else if(fish.y + fish.diameter < this.player.ymin - 400){
              fish.y+= 800;
        ...
        checkCollisions: function(){
          ...
          if(!collided){
            for(var i = this.lazers.length - 1; i >= 0; i--){
              ...
              if(lazer.active && this.rectRectCollision(this.player,lazer)){
                collided = true;
                this.updateBestScore();
                this.addPlayerExplosion();
                this.player.vy = -10;
          ...
          if(!collided && this.player.y + this.player.h + this.cam.y > this.lava.y1){
            this.updateBestScore();
            this.player.vy = -8;
            this.player.eyes.value = 0;
            this.player.mouth.value = 0;
            ...
            if(block.y > this.player.ymin + 400){
              ...
              if(dist(b1.x + b1.w, b1.y, b2.x, b2.y) > 200) {
                ...
                if(selection < 0.15 && b1.h > 170){
                  ...
                  spikeSide = random() < 0.5 ? "left" : "right";

                  this.spikes.push({
                    x: spikeSide === "left" ? b1.x + b1.w : b2.x,
                    y: random(b1.y + 30, b1.y + b1.h - 30),
                    diameter: 60,
                    angle: 0,
                    rot: random(1, 2) * (random() < 0.5 ? 1 : -1),
                  ...
                  var dir = random() < 0.5 ? 1 : -1;
                  this.enemies.push({
                    x: b1.x + b1.w + ((b2.x - (b1.x + b1.w)) / 2),
                    y: this.blocks.y + b1.h / 2,
                    diameter : 40,
                    vx: dir,
                    vy: 0,
                    dx: 0,
                ...
                if(!lazerAdded && random() < 0.75){
                  ...
                  for(var j = 0; j < numberOfStars; j++){
                    this.stars.push({
                      x: spikeSide === "right" ? b1.x + b1.w +18 : b2.x -18,
                      y: b1.y + b1.h / 2 -(numberOfStars - 1) * 20 + j * 40,
                      diameter: 30,
                      color: color(242, 215, 63),
                      image: this.starImages[random(this.starImages.length) | 0]
                    });
                  }
                }
              }
        ...
        drawSpikes: function() {
          for(var i = this.spikes.length - 1; i >= 0; i--){
            ...
            image(this.images.spike, -spike.diameter * 0.55, -spike.diameter * 0.55, spike.diameter * 1.1, spike.diameter * 1.1);
            popMatrix();
          ...
          for(var i = this.lazers.length - 1; i >= 0; i--){
            ...
            if(lazer.active) {
              if(random() < 0.8) {
                ...
                strokeWeight(3);
                stroke(255,150);
                line(lazer.x, lazer.y +5, lazer.x + lazer.w, lazer.y + 5)
              }
            }
            noStroke();
            fill(69,68,68);
            ellipse(lazer.x, lazer.y +5, lazer.diameter, lazer.diameter)
            ellipse(lazer.x + lazer.w + 10, lazer.y + 5, lazer.diameter, lazer.diameter);
        ...
        drawStars: function(){
          ...
          for(var i = this.stars.length - 1; i >= 0; i--){
            ...
            image(star.image, 0, 0, this.star.diameter * 1.5, star.diameter * 1.5);
            popMatrix();
        ...
        moveEnemies: function() {
          for(var i = this.enemies.length - 1; i >= 0; i--){
            ...
            enemy.angle.value = lerp(enemy.angle.value, enemy.angle.dest, 0.2);
        ...
        drawEnemies: function() {
          ...
          for(var i = this.enemies.length - 1; i >= 0; i--){
            ...
            rotate(radians(enemy.angle.value + sin(radians(frameCount * 10)) * 10));

            fill(247,208,67);
            triangle(enemy.diameter * 0.7, 0, enemy.diameter * 0.4, -enemy.diameter * 0.15, enemy.diameter * 0.4, enemy.diameter * 0.15);
            fill(enemy.color);
            ellipse(0,0, enemy.diameter, enemy.diameter);
            noStroke();
            fill(79,79,79);
            arc(0,0, enemy.diameter, enemy.diameter, 0, radians(180));
            noFill();
            stroke(255, 200);
            strokeWeight(1);
            ellipse(0,0, enemy.diameter, enemy.diameter);
            ...
            fill(247, 54, 74);
            triangle(-enemy.diameter / 2, 0, -enemy.diameter * 0.7, -enemy.diameter * 0.2, -enemy.diameter * 0.7, enemy.diameter * 0.2);

            fill(50, 150);
            ellipse(enemy.diameter * 0.3, -enemy.diameter * 0.15, enemy.diameter * 0.15, enemy.diameter * 0.15);
            fill(250, 100);
            ellipse(enemy.diameter * 0.3, enemy.diameter * 0.15, enemy.diameter * 0.15, enemy.diameter * 0.15);
            popMatrix();
        ...
        drawDeadPlayer: function(){
          ...
          pushMatrix();
          translate(this.player.x + this.player.w / 2, this.player.y + this.player.h / 2);
          ...
          if(this.player.y - this.player.h > this.player.ymin + 400){
            this.dead = true;
          }
        },
        drawLava: function() {
          pushStyle();
          stroke(lerpColor(color(242, 145, 97), color(232, 108, 46),map(sin(radians(frameCount * 4)), -1, 1, 1, 0)));
          strokeWeight(15);
          fill(242, 229, 111);
          beginShape();
              vertex(this.lava.x1, this.lava.y1);
              bezierVertex(
                this.lava.cx1 + cos(frameCount * 4) * 100, this.lava.cy1 + sin(radians(frameCount  * 4)) * 15,
                this.lava.cx2 + sin(frameCount * 4) * 100, this.lava.cy2 + cos(radians(frameCount  * 4)) * 15,
                this.lava.x2, this.lava.y2
              );
          ...
          for(var i = this.lava.arr.length - 1; i >= 0; i--){
            ...
            ellipse(particle.x, particle.y, particle.diameter, particle.diameter);
            particle.color = lerpColor(color(237, 205, 87),color(232,108,46), map(particle.timer, 0, 120, 0, 1));
        ...
        drawParticles: function(){
          ...
          for(var i = this.particles.length - 1; i >= 0; i--){
            ...
            rotate(radians(particle.angle));
                fill(particle.color, particle.opacity);
                rect(-particle.diameter / 2, -particle.diameter / 2,particle.diameter / 2, particle.diameter / 2);
            rotate(radians(-particle.angle));
            translate(-(particle.x + particle.diameter / 2), -(particle.y + particle.diameter / 2));
            ...
            particle.y+= particle.vy;
            particle.opacity = constrain(particle.opacity - 5, 0, 255);
        ...
        updateParallax: function(){
          this.parallax.back.y1 -= this.player.vy * this.parallax.back.speed;
          this.parallax.back.y2 -= this.player.vy * this.parallax.back.speed;

          if (this.parallax.back.y1 + this.cam.y >= this.images.back.height) {
            this.parallax.back.y2 = this.parallax.back.y1 - this.images.back.height;
          }
        },
        leaderboard: function() {
          ...
          image(this.images.leaderboard, 0, 0);
          ...
          pushStyle();
              ...
              for(var i = 0; i < this.highscores.length; i++){
                text(this.highscores[i].name, width / 2, 110 + i * 35);
              }
          popStyle();
          ...
          for(var k in this.buttons.leaderboard) {
            this.buttons.leaderboard[k].draw();
          }
        },
        home: function() {
          background(this.colors.backColor);
        ...
        gameover: function(){
          ...
          this.cam.y = lerp(this.cam.y, height * 0.7 - this.player.ymin, 0.1);
          ...
          if(this.dead) {
            this.anim.over.value = lerp(this.anim.over.value, 0, 0.1);
            this.anim.over.angle = map(this.anim.over.value, this.anim.over.base, 0, 0, 360);
